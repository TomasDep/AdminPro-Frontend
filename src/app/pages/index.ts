export * from './dashboard/dashboard.component';
export * from './account-settings/account-settings.component';
export * from './busqueda/busqueda.component';
export * from './mantenimientos/hospitales/hospitales.component';
export * from './mantenimientos/usuarios/usuarios.component';
export * from './mantenimientos/medicos/medicos.component';
export * from './mantenimientos/medicos/medico/medico.component';
export * from './perfil/perfil.component';
export * from './pages.component';

export * from './child-routes.module';
export * from './pages.module';

export * from './pages.routing';